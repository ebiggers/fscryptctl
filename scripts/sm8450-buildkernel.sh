#!/bin/bash
#
# Builds a kernel for the SM8450 HDK, and packs it up into a boot.img file that
# is set up to boot into the userdata partition ("root=/dev/sda11").  The root
# filesystem of a Linux distro should be flashed onto this partition first.
#
...
cp $SCRIPTDIR/sm8450.kconfig .config
$MAKE olddefconfig
$MAKE Image.gz dtbs
echo placeholder > ramdisk
cat arch/arm64/boot/Image.gz \
    arch/arm64/boot/dts/qcom/sm8450-hdk.dtb > Image.gz+dtb
...
echo "Created boot.img"
	...
	#ssh root@alarm "cp boot.img /dev/disk/by-partlabel/boot_a && \
			...
			#sync && reboot"
	#echo "Triggered reboot into new kernel"
#fi
