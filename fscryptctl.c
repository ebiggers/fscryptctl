...
// max_size bytes, from standard input into the provided buffer.  On success,
// returns the key size in bytes.  On failure, returns 0.
//
...
static size_t read_key(uint8_t *raw_key, int max_size) {
...
static int cmd_import_hw_wrapped_key(int argc, char *const argv[]) {
  ...
  uint8_t *longterm_wrapped_key = xzalloc(MAX_WRAPPED_KEY_SIZE);
  arg.raw_key_ptr = (uintptr_t)raw_key;
  arg.raw_key_size = read_key(raw_key, FSCRYPT_MAX_KEY_SIZE);
  if (arg.raw_key_size == 0) {
    goto cleanup;
  }
  arg.longterm_wrapped_key_ptr = (uintptr_t)longterm_wrapped_key;
  arg.longterm_wrapped_key_size = MAX_WRAPPED_KEY_SIZE;
  ...
  close(fd);
  if (!full_write(STDOUT_FILENO, longterm_wrapped_key,
...
cleanup:
  ...
  wipe_and_free(longterm_wrapped_key, MAX_WRAPPED_KEY_SIZE);
  return status;
}
...
static int cmd_prepare_hw_wrapped_key(int argc, char *const argv[]) {
  ...
  uint8_t *longterm_wrapped_key = xzalloc(MAX_WRAPPED_KEY_SIZE);
  uint8_t *ephemerally_wrapped_key = xzalloc(MAX_WRAPPED_KEY_SIZE);
  arg.longterm_wrapped_key_ptr = (uintptr_t)longterm_wrapped_key;
  arg.longterm_wrapped_key_size =
      read_key(longterm_wrapped_key, MAX_WRAPPED_KEY_SIZE);
  if (arg.longterm_wrapped_key_size == 0) {
    goto cleanup;
  }
  arg.ephemerally_wrapped_key_ptr = (uintptr_t)ephemerally_wrapped_key;
  arg.ephemerally_wrapped_key_size = MAX_WRAPPED_KEY_SIZE;
  ...
  close(fd);
  if (!full_write(STDOUT_FILENO, ephemerally_wrapped_key,
                  arg.ephemerally_wrapped_key_size)) {
    goto cleanup;
  }
  status = EXIT_SUCCESS;
cleanup:
  wipe_and_free(longterm_wrapped_key, FSCRYPT_MAX_KEY_SIZE);
  wipe_and_free(ephemerally_wrapped_key, MAX_WRAPPED_KEY_SIZE);
  return status;
}
